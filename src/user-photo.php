...
function userphoto_delete_photo($userID) {
    $userdata = get_userdata($userID);
 ...
 * @return mixed
 */
function cleanCropInfo($cropInfo) {

    $userphoto_maximum_dimension = get_option('userphoto_maximum_dimension');

    if($cropInfo['h'] == -1) {
        $cropInfo['h'] = $userphoto_maximum_dimension;
    }
    if($cropInfo['w'] == -1) {
        $cropInfo['w'] = $userphoto_maximum_dimension;
    }
    if($cropInfo['x'] == -1) {
        $cropInfo['x'] = 0;
    }
...
function userphoto_profile_update($userID) {
    ...
    if (@$_POST['userphoto_delete']) {
        ...
        if (isset($_FILES['userphoto_image_file']) && @$_FILES['userphoto_image_file']['name']) {
            ...
            if (!$error) {
                #
                # Resize the image so that it does not exceed any of the maximum dimensions
                #


                $crop_info = cleanCropInfo(array(
                    x => @$_POST['cropX'],
                    y => @$_POST['cropY'],
                    w => @$_POST['cropW'],
                    h => @$_POST['cropH']
                ));
                error_log("Got Crop Info: ".var_export($crop_info, true));
            ...
            if (!$error) {
                ...
                $user_photos_dir = trailingslashit($upload_dir['basedir']) . 'userphoto';
                if (!file_exists($user_photos_dir) && @!mkdir($user_photos_dir, 0777)){
                    $error = sprintf(__("The userphoto upload content directory does not exist and could not be created. Please ensure that you have write permissions for the '%s' directory. Did you put slash at the beginning of the upload path in Misc. settings? It should be a path relative to the WordPress root directory. <code>wp_upload_dir()</code> returned:<br /> <code style='white-space:pre'>%s</code>", 'user-photo'), $user_photos_dir, print_r($upload_dir, true));
                }
                ...
                if (!$error) {
                    ...
                    $imagepath = $user_photos_dir . '/' . $imagefile;
                    $thumbfile = preg_replace("/(?=\.\w+$)/", '.thumbnail', $imagefile);
                    $thumbpath = $user_photos_dir . '/' . $thumbfile;
                    ...
                    if (!move_uploaded_file($tmppath, $imagepath)) {
                        ...
                        if ($oldimagefile != $imagefile)
                            @unlink($user_photos_dir . '/' . $oldimagefile);
                        if ($oldthumbfile != $thumbfile)
                            @unlink($user_photos_dir . '/' . $oldthumbfile);
                    }
                }
            }
        }
...
<?php if ($isOldWP): ?>
    ...
    <?php else: ?>
		<table class='form-table' id="userphoto">
			<tr>
    <th>
        <!--<label for="userphoto_image_file">-->
            <?php echo $isSelf ? _e("Your Photo", 'user-photo') : _e("User Photo", 'user-photo') ?><!--</label>-->
    </th>
				<td>
	<?php endif; ?>
    ...
    ?>
    <p class='image'><img class='user-photo-image full-size'
                          src="<?php echo $bdir . $profileuser->userphoto_image_file . "?" . rand() ?>"
                          alt="Full size image"/><br/>
        Current User Photo (Full size)
    </p>
    <p class='image'><img class='user-photo-image thumbnail'
                          src="<?php echo $bdir . $profileuser->userphoto_thumb_file . "?" . rand() ?>"
                          alt="Thumbnail image"/><br/>
    ...
    <?php if (!$current_user->has_cap('edit_users')): ?>
        <?php if ($profileuser->userphoto_approvalstatus == USERPHOTO_PENDING): ?>
            ...
            <p id='userphoto-status-rejected'>
                <strong>Notice: </strong> <?php _e("Your chosen profile photo has been rejected.", 'user-photo') ?>
                <?php
    ...
    <div id="userphoto-upload-cropper">
        <p class='image'>
            <img class='user-photo-image preview' id="user-photo-preview" src="" alt="Preview"/><br/>
            User Photo Preview
        </p>
        <input type="hidden" name="cropX" value="-1"/>
        <input type="hidden" name="cropY" value="-1"/>
        <input type="hidden" name="cropW" value="-1"/>
        <input type="hidden" name="cropH" value="-1"/>

        <div id="userphoto-size-display" class="ok-size">
            Size: <span class="height"></span> x <span class="width"> (px)</span>
            <div class="warning">
                If the size is too small, there'll be stretching
                and it won't look great on the site.
            </div>
        </div>
    </div>
    ...
    <p id="userphoto-approvalstatus-controls" <?php if ($profileuser->userphoto_approvalstatus == USERPHOTO_PENDING) echo "class='pending'" ?>>
        <label><?php _e("Approval status:", 'user-photo') ?>
            <select name="userphoto_approvalstatus" id="userphoto_approvalstatus"
                    onchange="userphoto_approvalstatus_onchange()">
    ...
    <?php endif; ?>
    <?php if ($profileuser->userphoto_image_file): ?>
    <p><label><input type="checkbox" name="userphoto_delete" id="userphoto_delete"
                     onclick="userphoto_onclick()"/> <?php _e('Delete photo?', 'user-photo')?></label></p>
    <?php endif; ?>
...
<div class="wrap">
    ...
    <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" id='userphoto_options_form'>
        ...
        <?php echo $betweenRow ?>
        <input type="number" min="1" step="1" size="3" id="userphoto_maximum_dimension"
               name="userphoto_maximum_dimension" value="<?php echo $userphoto_maximum_dimension ?>"/>px
        <?php echo $afterRow ?>
        <?php echo $beforeRow ?>
        ...
        <?php echo $betweenRow ?>
        <input type="number" min="1" step="1" size="3" id="userphoto_thumb_dimension" name="userphoto_thumb_dimension"
               value="<?php echo $userphoto_thumb_dimension ?>"/>px
        <?php echo $afterRow ?>
        <?php echo $beforeRow ?>
        ...
        <?php echo $betweenRow ?>
        <input type="checkbox" value='Y' id="userphoto_use_avatar_fallback"
               name="userphoto_use_avatar_fallback" <?php if ($userphoto_use_avatar_fallback) echo ' checked="checked"'; ?> /><br/>
        ...
        <?php echo $betweenRow ?>
        <input type="checkbox" value='Y' id="userphoto_override_avatar"
               name="userphoto_override_avatar" <?php if ($userphoto_override_avatar) echo ' checked="checked"'; ?> /><br/>
        ...
        <?php echo $betweenRow ?>
        <input type="range" min="1" max="100" step="1" size="3" id="userphoto_jpeg_compression"
        ...
        <select name="userphoto_level_moderated" id="userphoto_level_moderated">
            <option value="-1" <?php if ($userphoto_level_moderated == -1) echo ' selected="selected"' ?> class='none'>
                (none)
            </option>
            <option value="0" <?php if ($userphoto_level_moderated == 0) echo ' selected="selected"' ?>>Subscriber
            </option>
            <option value="1" <?php if ($userphoto_level_moderated == 1) echo ' selected="selected"' ?>>Contributor
            </option>
        ...
        <input type="hidden" name="page_options"
               value="userphoto_jpeg_compression,userphoto_admin_notified,userphoto_maximum_dimension,userphoto_thumb_dimension,userphoto_level_moderated"/>
...
function userphoto_resize_image($filename, $newFilename, $cropDetails, &$error) {
    if (!$newFilename)
